...
async function readFileExecute(args: { path: string}): Promise<string> {
  ...
  try {
    ...
    if (stat.size > MAX_FILE_SIZE) {
      throw new Error("ファイルが大きすぎます。100kb以下のファイルを読み込めます")
    } 
  } catch (error: any) {
...
export const readFile = {
  name: "readFile",
  // LLMが判断をしやすい内容を心がける。
  description: "ワークスペース内の指定されたパスのファイル内容を文字列として読み込む。ファイルが存在しない場合はエラーを返す。100kbを超える巨大なファイルは読み込めない。相対パスまたは絶対パスを指定できる。",
  parameters: {
    type: "object",
    properties: {
      path: {
